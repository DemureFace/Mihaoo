  ...
  <div class="wrap">
    <header class="top">
      ...
      <div class="row">
        ...
        <div class="tabs">
          <button class="tab" :class="{active: tab==='content'}" @click="tab='content'">Content</button>
          <button class="tab" :class="{active: tab==='data'}" @click="tab='data'">Data</button>
          <button class="tab" :class="{active: tab==='snippet'}" @click="tab='snippet'">Snippet</button>
        </div>
    ...
    <main class="grid">
      <section class="card">
        ...
        />
        <p class="hint">
          Заміна йде тільки для валютних патернів (символ/код + число), як у твоєму процесорі:contentReference[oaicite:3]{index=3}.
        </p>
      </section>
      ...
      <section class="card">
        ...
        <p class="hint">
          Site locales: <b>{{ (siteModes[site] || []).join(", ") }}</b>
        </p>
      </section>
    </main>
  </div>
...
const defaultFormatLocaleByTarget = { de:"de","en-au":"au",et:"et",fi:"fi",fr:"fr",it:"it",no:"no",el:"gr",pt:"pt" };
...
const conversionRules = {
  ...
  et:{rate:1,symbol:"€",position:"after",separator:" ",decimal:",",spaceBefore:true},
  fi:{rate:1,symbol:"€",position:"after",separator:" ",decimal:",",spaceBefore:true},
...
function processText(text, locale) {
  const pattern = /([€$£])\s?(\d[\d.,\s]*)|(\d[\d.,\s]*)\s*(€|\$|£|kr|eur|usd|cad|aud|nzd|nok|chf|gbp)/gi;
...
function eurSnippet(intPart, fracPart) {
  ...
  if (fracPart) {
    if (intPart === "0" || !intPart) return `<Components.Snippet templateName="currencies-0${fracPart}eur" />`;
...
function processTextForSnippets(text) {
  ...
  const PATTERN = new RegExp(`(?:(${CURRENCY_TOKEN})[ \\t\\u00A0]*(${NUM_TOKEN})|(${NUM_TOKEN})[ \\t\\u00A0]*(${CURRENCY_TOKEN}))`, "gi");
...
<style scoped>
.wrap{min-height:100vh;background:#f6f7fb;padding:20px}
.top{position:sticky;top:0;background:#f6f7fb;padding-bottom:14px;z-index:5}
h1{margin:0 0 12px;font-size:20px}
.row{display:flex;gap:12px;flex-wrap:wrap;align-items:end}
.field{display:flex;flex-direction:column;gap:6px}
select{height:38px;border:1px solid #d7dbe7;border-radius:10px;padding:0 10px;background:#fff}
.tabs{display:flex;gap:8px;margin-left:auto}
.tab{height:38px;border-radius:10px;border:1px solid #d7dbe7;background:#fff;padding:0 12px;cursor:pointer}
.tab.active{background:#111827;color:#fff;border-color:#111827}
.grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}
@media (max-width: 980px){.grid{grid-template-columns:1fr}}
.card{background:#fff;border:1px solid #e3e7f2;border-radius:16px;padding:14px}
.cardHdr{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}
h2{margin:0;font-size:16px}
.ta{width:100%;border:1px solid #d7dbe7;border-radius:12px;padding:12px;font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace}
.btn{height:34px;border-radius:10px;border:1px solid #d7dbe7;background:#fff;padding:0 12px;cursor:pointer}
.btn:disabled{opacity:.5;cursor:not-allowed}
.hint{margin:10px 0 0;color:#6b7280;font-size:12px}
</style>
